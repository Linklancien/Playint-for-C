...
typedef struct {
	...
	unsigned int *new_actions_linked_array;
...
void *playint_Context_init(void *userpointer, unsigned int keyslinks_len, unsigned int todolist_cap, unsigned int mode_len){
    ...
    context->keyslinks_array_len = keyslinks_len;

    mode_array = malloc((sizeof *mode_array)*mode_len);
    interaction_array = malloc((sizeof *interaction_array)*todolist_cap);
    new_actions_linked_array = malloc((sizeof *new_actions_linked_array)*todolist_cap);
    ...
    context->todolist.new_actions_linked_array = new_actions_linked_array;
...
void playint_Context_action_add(playint_Context *context, char *name, playint_UserFunction function_pointer){
    context->actions_array = realloc(context->actions_array, sizeof(*context->actions_array)*context->actions_len);
    context->actions_array[context->actions_len].name = name;
    context->actions_array[context->actions_len].function_pointer_list = function_pointer;
...
char *playint_Context_action_get_name_by_id(playint_Context *context, unsigned int id){
    ...
    return context->actions_array[id].name;
}
...
unsigned int *playint_Context_action_get_id_by_name(playint_Context *context, char *name){
    ...
    for (i=0; i < context->actions_len ; i ++ ){
        if (context->actions_array[i].name == name){
            current_id += 1;
...
void playint_Context_todoarray_change_idnext(playint_Context *context){
    ...
    if (context->todolist.idnext + 1 == context->todolist.cap){
        ...
        context->todolist.new_actions_linked_array = realloc(context->todolist.new_actions_linked_array, (sizeof *context->todolist.new_actions_linked_array)*context->todolist.cap);
        ...
        for (i = 0; i < context->todolist.idnext; i++){
            ...
            context->todolist.new_actions_linked_array[i+old_cap] = context->todolist.new_actions_linked_array[i] ;
        }
...
void playint_Context_todoarray_add(playint_Context *context, int id_pressed){
    ...
    if (context->state == changed){
        context->todolist.new_actions_linked_array[context->todolist.idnext] = context->action_to_link;
    }
...
void playint_Context_todoarray_do_one(playint_Context *context){
    ...
    if (context->todolist.idstart != context->todolist.idnext){
        ...
        if (interaction.type == activated){
            ...
            context->actions_array[id_action].function_pointer_list(context->userpointer);
        }
        else if (interaction.type == changed){
            playint_Context_mode_keyslinks_change_at(context, int mode_id, interaction.id_pressed, context->todolist.new_actions_linked_array[context->todolist.idstart]);
        }
    ...
    if (context->todolist.idstart < context->todolist.idnext){
        for (i = context->todolist.idstart; i < context->todolist.idnext ; i++){
            ...
            if (interaction.type == activated){
                ...
                playint_Context_mode_keyslinks_change_at(context, int mode_id, interaction.id_pressed, context->todolist.new_actions_linked_array[i]);
            }
        }
    }
    else{
